...
interface ProductListProps {
  ...
  assignToMultipleBoxes?: (product: Product | number, amountPerBox: number, numberOfBoxes: number) => void;
}
    ...
    <div className="w-full sm:w-2/12 md:w-3/12 lg:w-1/4 xl:w-1/5 bg-gray-50 rounded-xl p-4 flex flex-col gap-3 shadow-inner max-h-full">
      ...
      <AssignItemsModal
        ...
        onAssignToOneBox={(boxId: number, amount: number) => {
          ...
          // Preferimos assignToBox (metodo centralizado que añade y descuenta)
          if (assignToBox) {
            assignToBox(selectedProduct, boxId, amount);
          } else {
            // fallback: solo descontar (no ideal, pero mantenemos compatibilidad)
